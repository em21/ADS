...
public class BSTree {
    ...
    private void insert(String s,Node node){
		if (s.compareTo(node) < 0){
		    ...
		    else{newNode = new Node(s,null,null,node);
    ...
    private Node find(String s,Node node){
	    ...
	    if (s.compareTo(node.getElement())<0){return find(s, node.getLeft());}//something to do with getting child 
	    if (s.compareTo(node.getElement())>0){return find(s, node.getRight());}//look at later
		else {return node;}
	    }
